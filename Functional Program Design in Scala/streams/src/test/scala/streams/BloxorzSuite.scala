...
class BloxorzSuite extends FunSuite {
  ...
  test("isLegal implementation"){
    new Level1 {
      assert(startBlock.isLegal === true)
      assert(Block(Pos(1,2),Pos(1,3)).isLegal === true)
      assert(Block(Pos(-1,2),Pos(1,3)).isLegal === false)
    }
  }


  test("legalNeighbors implementation"){
    new Level1 {
      assert(startBlock.legalNeighbors === List((Block(Pos(1,2),Pos(1,3)), Right),(Block(Pos(2,1),Pos(3,1)),Down)))
    }
  }

  test("neighborsWithHistory implementation"){
    new Level1 {
