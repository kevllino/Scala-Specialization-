...
//  test("'makeIndex' creates a simple index with two entries - 2") {
//    assert(initializeWikipediaRanking(), " -- did you fill in all the values in WikipediaRanking (conf, sc, wikiRdd)?")
//    import WikipediaRanking._
//    val langs = List("Scala", "Java")
//    val art1 = WikipediaArticle("1","Groovy is pretty interesting, and so is Erlang")
//    val art2 = WikipediaArticle("2"," Java run on the JVM")
//    val art3 = WikipediaArticle("3","Scala is not purely functional")
//    val articles = List(art1, art2, art3)
//    val rdd = sc.parallelize(articles)
//    val index:RDD[(String, Iterable[WikipediaArticle])] = makeIndex(langs, rdd)
//    assert(index.collect().sortBy(_._1) === Array(("Java", Iterable(art2), ("Scala", Iterable(art3)))))
//  }
