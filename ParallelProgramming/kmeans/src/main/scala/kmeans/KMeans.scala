...
class KMeans {
  ...
  // return mean which is the closes to the point
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val moins: GenMap[Point, GenSeq[Point]] = points.par.groupBy(findClosest(_, means))
    means.par.map{ el =>
      el -> moins.getOrElse(el, Nil)
    }.toMap
  }
  ...
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    if (!converged(eta)(means, update(classify(points, means),means))) kMeans(points, update(classify(points, means),means), eta) else update(classify(points, means),means) // your implementation need to be tail recursive
  }
}
