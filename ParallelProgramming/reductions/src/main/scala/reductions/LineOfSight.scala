...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    ...
    while(i < input.length){
      output(i) = Math.max(output.max, input(i) / i )
      i = i + 1
    }
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    val angles = for (i <- from until until)
      yield input(i) / i
    angles.max
  }
...
//  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    if (from < until) {
      val mx = max(startingAngle, input(from) / from)
      output(from) = mx
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float,
    ...
    case Leaf(f, u, mxprev) => downsweepSequential(input, output, startingAngle, f, u)
    case Node(l, r) => {
  ...
  def parLineOfSight(input: Array[Float], output: Array[Float],
    threshold: Int): Unit = {
    val uTree = upsweep(input, 1, input.length, threshold)
    downsweep(input, output, 0 , uTree)
  }
}
