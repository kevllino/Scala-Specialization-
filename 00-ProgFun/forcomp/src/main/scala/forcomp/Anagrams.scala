...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences =  w.toLowerCase.groupBy(c => c).map(el => (el._1, el._2.length)).toList.sorted
  ...
  // occurences of characters the words in Sentence
  // combinations with those characters so that words are from dictionnary
