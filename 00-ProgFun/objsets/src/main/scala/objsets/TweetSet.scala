...
abstract class TweetSet {
  ...
  def descendingByRetweet: TweetList = this match {
    case _: Empty => Nil
    case _: NonEmpty => new Cons (mostRetweeted, remove (mostRetweeted).descendingByRetweet)
  }
...
object GoogleVsApple {
  ...
  def filterTweets(collection: List[String]): TweetSet = allTweets.filter(tweet =>  collection.exists( el => tweet.text.contains(el) ))
